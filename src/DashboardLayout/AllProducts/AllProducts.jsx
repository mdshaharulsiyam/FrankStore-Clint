                    ...
                    <tbody>
                        {
                            allproducts.map(item => <tr key={item._id} item={item}>
                                ...
                                <td className={`${item?.quantity == 0 ? "text-red-400 font-bold" :(item?.quantity > 0 && item?.quantity < 20)?'text-orange-500 font-bold' : 'text-black'} `}>{item?.quantity == 0 ? 'out of stock' : (item?.quantity > 0 && item?.quantity < 20) ?`low stock (${item?.quantity})` :item?.quantity}</td>
